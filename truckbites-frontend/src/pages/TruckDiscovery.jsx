...
export default function TruckDiscovery() {
  ...
  // Enrich trucks with distance and sort nearest-first when location is active
  ...
  const filteredTrucks = useMemo(() => {
    if (!searchQuery.trim()) return trucksWithDistance;
    const query = searchQuery.toLowerCase();
    return trucksWithDistance.filter(
      (t) =>
        t.name.toLowerCase().includes(query) ||
        t.cuisineType.toLowerCase().includes(query) ||
        (t.description && t.description.toLowerCase().includes(query))
    );
  }, [trucksWithDistance, searchQuery]);
    ...
    <div className="min-h-[80vh]">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          {(searchQuery || cuisineFilter !== 'All' || location) && (
            <button
              onClick={() => {
                setSearchQuery('');
